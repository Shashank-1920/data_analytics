...
def _classify_customer_segment(total_orders, avg_gap, last_order_date, first_order_date):
    """
    New segmentation: Loyal, At Risk, Lost, New, High Value, One-time buyers
    """
    try:
        # Handle NaN values
        if total_orders is None or (isinstance(total_orders, float) and math.isnan(total_orders)):
            return 'New'
        ...
        if pd.notna(last_order_date):
            last_date = pd.Timestamp(last_order_date)
            days_since_last = (datetime.now().replace(tzinfo=None) - last_date.replace(tzinfo=None)).days
        else:
            days_since_last = float('inf')
        
        # Calculate customer lifetime (days from first to last order)
        if pd.notna(first_order_date) and pd.notna(last_order_date):
            first_date = pd.Timestamp(first_order_date)
            customer_lifetime = (last_date.replace(tzinfo=None) - first_date.replace(tzinfo=None)).days
        else:
            customer_lifetime = 0
        
        # Handle avg_gap NaN
        if avg_gap is None or (isinstance(avg_gap, float) and math.isnan(avg_gap)):
            avg_gap = float('inf')
        else:
        ...
        # New customers (very recent, few orders)
        if customer_lifetime < 30 and total_orders <= 2:
            return 'New'
        
        # High Value (frequent orders and many total orders)
        if total_orders >= 10 and avg_gap <= 14:
            return 'High Value'
        
        # Loyal (consistent ordering, healthy frequency)
        if total_orders >= 5 and avg_gap > 0 and days_since_last <= avg_gap * 1.5:
            return 'Loyal'
        
        # At Risk (used to order regularly but haven't lately)
        if total_orders >= 3 and days_since_last > avg_gap * 1.5 and days_since_last < avg_gap * 3:
            return 'At Risk'
        
        # Lost (no activity for a long time or infrequent orders)
        if days_since_last > avg_gap * 3 or days_since_last > 90:
            return 'Lost'
        
        # Default to Loyal if regular ordering
        if total_orders >= 3:
            return 'Loyal'
...
def perform_analytics(connection, table_name):
    ...
    results['recency'] = results['last_order_date'].apply(lambda x: (datetime.now().replace(tzinfo=None) - pd.Timestamp(x).replace(tzinfo=None)).days if pd.notna(x) else None)
    
    # Calculate new segmentation classification FIRST
    results['customer_classification'] = results.apply(
        lambda row: _classify_customer_segment(
            row['total_orders'],
            row['avg_order_gap'],
            row['last_order_date'],
    ...
    # Calculate churn_flag (1 if at risk or lost, 0 otherwise) AFTER segmentation
    results['churn_flag'] = results.apply(
        lambda row: 1 if row['customer_classification'] in ['At Risk', 'Lost'] else 0,
        axis=1
    )
